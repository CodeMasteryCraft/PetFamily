...
public class CreatePetRequestValidator : AbstractValidator<CreatePetRequest>
{
    public CreatePetRequestValidator()
    {
        ...
        RuleFor(x => x.BirthDate.Year).
            GreaterThan(Constraints.YEAR1900);
        ...
        RuleFor(x => x.Building).
            MinimumLength(Constraints.MINIMUM_TITLE_LENGTH).
            MaximumLength(Constraints.SHORT_TITLE_LENGTH);
        RuleFor(x => x.Index.Length).
            Equal(Constraints.INDEX_TITLE_LENGTH);
        ...
        RuleFor(x => x.VolunteerPhoneNumber).
            MinimumLength(Constraints.MINIMUM_TITLE_LENGTH).
            MaximumLength(Constraints.SHORT_TITLE_LENGTH);

        RuleFor(x => x.CreatedDate).
            LessThan(DateTimeOffset.UtcNow);
        RuleFor(x => x.CreatedDate.Year).
            GreaterThan(Constraints.YEAR1900);
    }
}